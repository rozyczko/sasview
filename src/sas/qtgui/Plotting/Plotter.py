    ...
    def plot(self, data=None, color=None, marker=None, hide_error=False, transform=True):
        ...
        for ax in [self.ax, self.tpw_ax]:
            ...
            if version.parse(mpl.__version__) < version.parse("3.3"):
                ax.set_xscale(self.xscale, nonposx='clip') if self.xscale != 'linear' else ax.set_xscale(self.xscale)
                ax.set_yscale(self.yscale, nonposy='clip') if self.yscale != 'linear' else ax.set_yscale(self.yscale)
            else:
                ax.set_xscale(self.xscale, nonpositive='clip') if self.xscale != 'linear' else ax.set_xscale(self.xscale)
                ax.set_yscale(self.yscale, nonpositive='clip') if self.yscale != 'linear' else ax.set_yscale(self.yscale)

            # Draw non-standard markers
            l_width = markersize * 0.4
            if marker == '-' or marker == '--':
                line = ax.plot(x, y, color=color, lw=l_width, marker='',
                                 linestyle=marker, label=label, zorder=10)[0]

            elif marker == 'vline':
                y_min = min(y)*9.0/10.0 if min(y) < 0 else 0.0
                line = ax.vlines(x=x, ymin=y_min, ymax=y, color=color,
                                linestyle='-', label=label, lw=l_width, zorder=1)

            elif marker == 'step':
                line = ax.step(x, y, color=color, marker='', linestyle='-',
            ...
            if isinstance(self.setRange, SetGraphRange) and self.setRange.rangeModified:
                ...
                if isinstance(data, Data1D):
                    ...
                    default_x_range = ax.get_xlim()
                    default_y_range = ax.get_ylim()
